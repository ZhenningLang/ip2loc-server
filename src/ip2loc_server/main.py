...
def load_config(config_path_name: str):
    ...
    if not os.path.isfile(module.LOG_FILE_PATH):
        log_path_name = os.path.join(module.LOG_FILE_PATH, 'ip2loc.log')
    else:
        log_path_name = module.LOG_FILE_PATH
    CONFIG.LOGGING = EasyDict(
        LOG_LEVEL=module.LOG_LEVEL, LOG_FILE_PATH=log_path_name, LOG_FORMAT=module.LOG_FORMAT)
...
def setup_logger():
    logging.basicConfig(
        level=CONFIG.LOGGING.LOG_LEVEL, format=CONFIG.LOGGING.LOG_FORMAT,
        handlers=[logging.FileHandler(CONFIG.LOGGING.LOG_FILE_PATH), logging.StreamHandler()])
...
def _display_path(args: argparse.Namespace):
    ...
    cs_log = NOT_SPECIFIED if as_config == NOT_SPECIFIED else CONFIG.LOGGING.LOG_FILE_PATH
...
def display_path(args: argparse.Namespace):
    ...
    if args.showpath:
        ...
        logging.info(f'LOG FILE: {CONFIG.LOGGING.LOG_FILE_PATH}')
